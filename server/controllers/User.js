...
export const deleteWorkout = async (req, res, next) => {
  try {
    ...
    if (!id) {
      return next(createError(400, "Workout ID is required"));
    }

    const workout = await Workout.findOneAndDelete({ _id: id, user: userId });
    if (!workout) {
      return next(createError(404, "Workout not found or you don't have permission to delete it"));
    }
...
export const updateWorkout = async (req, res, next) => {
  try {
    ...
    const allowedUpdates = ['workoutName', 'category', 'sets', 'reps', 'weight', 'duration', 'caloriesBurned'];
    const actualUpdates = {};
    ...
    if (!workout) {
      return next(createError(404, "Workout not found or you don't have permission to update it"));
    }
